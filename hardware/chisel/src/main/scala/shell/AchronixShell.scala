 ...
 */
class AchronixShell(implicit p: Parameters) extends RawModule {
  ...
  val m_axi_gmem = IO(new ACXAXIMaster(mp))
  val s_axi_control = IO(new ACXAXIClient(hp))
  ...
  shell.io.mem.b.bits.user <> DontCare
  ...
  shell.io.mem.r.bits.user <> DontCare
